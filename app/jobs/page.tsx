...
import { useSavedJobStore, useIntegrationStore } from "@/lib/store";
...
const SOURCE_META = {
  remotive: { label: "Remotive", color: "#0d9488" },
...
export default function JobsPage() {
  ...
  const { settings: integration } = useIntegrationStore();
  ...
  if (integration.remotive) activeSources.push("remotive");
  if (integration.linkedin) activeSources.push("linkedin");
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      params.set("linkedinLocation", integration.linkedinLocation || "Remote");
    ...
    return (
      <motion.div key={job.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="flex items-center gap-1 text-[12px]" style={{ color: "var(--color-muted)" }}>
        ...
        {job.description && (
          <p className="text-[12.5px] line-clamp-2" style={{ color: "var(--color-muted)" }}>{job.description}</p>
        )}

        <div className="flex items-center gap-2 mt-auto pt-1">
          <Button size="sm" onClick={() => openProposal(job)}>
            <Icon name="send" size={13} className="mr-1.5" /> Buat Proposal
          </Button>
          {job.url && (
  ...
  return (
    <ShellLayout>
      <PageHeader
        title="Jobs"
        subtitle="Cari peluang freelance & remote dari Remotive dan LinkedIn"
        actions={
          <Link href="/integrations">
            <Button variant="outline" size="sm"><Icon name="settings" size={13} className="mr-1.5" /> Atur Sumber</Button>
          </Link>
        }
      />
      ...
      <div className="flex items-center gap-2 mb-5">
        <span className="text-[12px]" style={{ color: "var(--color-muted)" }}>Sumber aktif:</span>
        {activeSources.length === 0 ? (
          <Link href="/integrations" className="text-[12px] font-semibold" style={{ color: "#C64545" }}>
            Tidak ada sumber aktif — Atur di Integrasi
          </Link>
        ) : activeSources.map((s) => {
          const meta = SOURCE_META[s as keyof typeof SOURCE_META];
          return (
            <span key={s} className="flex items-center gap-1.5 text-[12px] font-semibold px-2.5 py-1 rounded-full"
      ...
      <div className="flex items-center gap-1 mb-5 p-1 rounded-[12px] w-fit" style={{ background: "var(--color-canvas)" }}>
        {([["search", "Cari Job"], ["saved", `Tersimpan${savedJobs.length ? ` (${savedJobs.length})` : ""}`]] as const).map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)}
            ...
            style={tab === key
              ? { background: "var(--color-surface)", color: "var(--color-primary-ink)", boxShadow: "var(--shadow-card)" }
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-2.5 mb-5">
            <Input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Cari: react developer, ui designer, copywriter…"
              className="flex-1 bg-[var(--color-surface-card)]"
              onKeyDown={(e) => e.key === "Enter" && fetchJobs()} />
            <Select value={category} onChange={(e) => setCategory(e.target.value)} className="sm:w-56">
              {CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
            </Select>
            <Button onClick={fetchJobs} disabled={loading || activeSources.length === 0}>
          ...
          {!searched ? (
            <div className="rounded-[14px] border border-dashed flex flex-col items-center text-center py-16 px-6" style={{ borderColor: "var(--color-hairline)" }}>
              ...
              <p className="text-[13px]" style={{ color: "var(--color-muted)" }}>Masukkan kata kunci, pilih kategori, lalu klik Cari.</p>
            </div>
          ) : loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {savedJobs.map((job) => (
              <div key={job.id} className="rounded-[14px] border p-5 flex items-start justify-between gap-3" style={{ background: "var(--color-surface-card)", borderColor: "var(--color-hairline)" }}>
                ...
                <button onClick={() => toggleJob({ id: job.id, title: job.title, company: job.company, location: job.location, url: job.url, category: job.category })}
                  className="p-1.5 rounded-lg flex-shrink-0" style={{ color: "var(--color-primary)" }}>
                  <Icon name="star" size={16} />
                </button>
              </div>
            ))}
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="flex flex-col gap-3 px-6 pb-6 pt-4 max-h-[70vh] overflow-y-auto">
            ...
            <Button onClick={generateProposal} disabled={generating}>
              {generating
                ? <><Icon name="spinner" size={14} spin className="mr-1.5" /> Membuat…</>
                : <><Icon name="sparkles" size={14} className="mr-1.5" /> Generate Proposal</>}
            </Button>

            <AnimatePresence>
              {proposal && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  className="rounded-[12px] border p-4" style={{ borderColor: "var(--color-hairline)", background: "var(--color-canvas)" }}>
