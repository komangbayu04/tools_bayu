...
export default function DashboardPage() {
  ...
  const projectColor = (id: string) => project(id)?.color ?? "#2A9D8F";
    ...
    <ShellLayout>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "rgba(42,157,143,0.10)" }}>
              <Icon name="list-check" size={15} style={{ color: "#2A9D8F" }} />
            </div>
            <p className="text-[15px] font-bold" style={{ color: "var(--color-ink)" }}>Task Saya</p>
          </div>
          <Link href="/todo/all" className="text-[12px] font-semibold flex items-center gap-1 hover:opacity-70" style={{ color: "#2A9D8F" }}>
                ...
                <button
                  ...
                  style={
                    active
                      ...
                      : { color: isToday ? "#2A9D8F" : "var(--color-muted)" }
                  }
                >
